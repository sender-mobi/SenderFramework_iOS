 ...
 * As far as I know, any other way of importing and using public keys from the outside is not recommended: https://devforums.apple.com/message/301532#301532
 */
class CryptoExportImportManager: NSObject {
    ...
    func importPublicKeyReferenceFromDERCertificate(_ certData: Data) -> SecKey? {
        ...
        var resultType: SecTrustResultType = UInt32(0) as! SecTrustResultType // result will be ignored.
    ...
    func exportRSAPublicKeyToDER(_ rawPublicKeyBytes: Data, keyType: String, keySize: Int) -> Data {
        ...
        let headerBuffer = headerData.mutableBytes.assumingMemoryBound(to: UInt8.self)
        ...
        let bitstringBuffer = bitstringData.mutableBytes.assumingMemoryBound(to: UInt8.self)
        bitstringBuffer[0] = kCryptoExportImportManagerASNHeaderBitstringMark // key length mark
        let keyLengthBytesEncoded = encodeASN1LengthParameter(rawPublicKeyBytes.count+1, buffer: &(bitstringBuffer[1]))
        ...
        derKey.append(kCryptoExportImportManagerRSAOIDHeader, length: kCryptoExportImportManagerRSAOIDHeaderLength) // Add OID header
    ...
    func createSecureKeyPair(_ keyTag: String, keySize: Int, completion: ((_ success: Bool, _ pubKeyData: Data?) -> Void)? = nil) {
        ...
        DispatchQueue.global(priority: DispatchQueue.GlobalQueuePriority.default).async { () -> Void in
            ...
            if status == errSecSuccess {
                DispatchQueue.main.async(execute: {
                    ...
                    let publicKeyData = self.getPublicKeyData(keyTag)
